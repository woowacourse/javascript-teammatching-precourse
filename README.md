...
- 코스 선택 라디오를 클릭하면 해당 코스에 크루를 추가할 수 있는 폼과 해당 코스 현재 크루의 테이블을 표시한다.
- 크루 입력 폼에 이름을 입력하고 제출하면 검증을 거친 뒤 리스트에 추가한다.
- 삭제 버튼을 클릭하면 `confirm`을 통해서 재확인하고 확인 시 삭제한다.

## 2. 팀 관리 탭

- 팀 관리 탭을 선택하면 코스와 미션을 선택할 수 있는 `select`가 표시된다.
- 코스와 미션을 선택한 후 버튼을 누르면 팀 매칭 존재 여부에 따라 매칭 결과나 매칭 폼이 표시된다.
- 팀 매칭이 없는 경우 1 팀 당 제한 인원 수를 입력하는 폼과 제출 버튼이 표시된다.
- 버튼을 클릭하면 자동으로 팀을 매칭한 뒤 결과를 표시한다.
- 결과에 담겨 있는 재매칭 버튼을 누르면 현재 매칭 결과를 삭제하고 다시 팀 매칭 폼을 표시한다.

# 예외 목록

## 1. 크루 입력

- 크루의 이름이 1~5글자가 아닌 경우

## 2. 팀 최소인원 입력

- 팀 최소 인원의 수가 현재 해당 코스의 크루원의 숫자보다 큰 경우
