import { ELEMENT_ID, TAP_BUTTON_ID } from '../constants.js';
...
export default class CrewTapView extends TapView {
  ...
  <main>
    <section>
      ...
      <div>
        <input type="radio" name="course" value="frontend" id=${ELEMENT_ID.radioFront}/>
        <label for="frontend">프론트엔드</label>
        <input type="radio" name="course" value="backend" id=${ELEMENT_ID.radioBack}/>
  ...
  teamView(radioValue) {
    ...
    <form>
      <label>크루 이름</label>
      <input type="text" id=${ELEMENT_ID.crewName}/>
      <button id=${ELEMENT_ID.crewButton}>확인</button>
    </form>
    `;
  }
  ...
  crewListView(radioValue) {
    ...
    <h3>프론트엔드 크루 목록</h3>
    <table border="1" id=${ELEMENT_ID.crewTable}>
      <thead>
        <tr>
          <th></th>
          <th>크루</th>
