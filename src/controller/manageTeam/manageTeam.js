...
const getShuffledTeam = (teamMemberCount) => {
    ...
    const randoms = MissionUtils.Random.shuffle([...Array(crews.length)].map((v, i) => i)).map(
        (idx) => crews[idx],
    );

    return makeResultForm(crews.length, teamMemberCount).map((count) => randoms.splice(0, count));
};
